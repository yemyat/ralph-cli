...
## Tasks
- [ ] Update test expectations in `templates.test.ts` to use `.ralph-wiggum/specs` instead of `.ralph/specs`
- [ ] Remove test for `git tag` since it's no longer in PROMPT_BUILD template
- [ ] Update IMPLEMENTATION_PLAN_TEMPLATE test to check for new sections (In Progress, Backlog, Completed) instead of old ones (Current Status, High/Medium/Low Priority, Discovered Issues)
- [ ] Remove checkbox syntax test since template now uses HTML comments instead

## Acceptance Criteria
- [ ] AC 1: `bun run test` passes with 0 failures
- [ ] AC 2: All assertions in `templates.test.ts` match actual template content
- [ ] AC 3: No test logic removed that still applies to current templates
